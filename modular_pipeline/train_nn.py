...
if __name__ == '__main__':
    ...
    if 'feature_subsets' in data and data['feature_subsets']:
        cols = data['feature_subsets'].get('nn')
        if cols is not None:
            X_num = X_num[cols]
    ...
    for run_idx in range(CFG.ensemble_runs):
        ...
        for seed in CFG.random_seeds:
            ...
            for fi, (tr_idx, val_idx) in enumerate(cv.split(X_scaled, y), 1):
                ...
                print(f'  [nn{tag}] {fold_num:2d}/{total} | s={effective_seed} f={fi} | acc={acc:.4f}')
        ...
        if CFG.ensemble_runs > 1:
            ...
            np.save(CFG.output_dir / f'oof_nn_r{run_idx}.npy', oof_probs)
            np.save(CFG.output_dir / f'test_nn_r{run_idx}.npy', avg_test)
    ...
    CFG.output_dir.mkdir(parents=True, exist_ok=True)
    np.save(CFG.output_dir / 'oof_nn.npy', final_oof)
    np.save(CFG.output_dir / 'test_nn.npy', final_test)

    final_acc = accuracy_score(y, final_oof >= 0.5)
    n_runs = CFG.ensemble_runs
    print(f'\n[OK] nn saved ({n_runs} run{"s" if n_runs > 1 else ""} avg), OOF acc: {final_acc:.5f}')
